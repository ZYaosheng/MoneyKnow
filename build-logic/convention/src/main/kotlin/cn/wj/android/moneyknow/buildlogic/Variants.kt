...
fun configureFlavors(
    ...
    flavorConfigurationBlock: ProductFlavor.(flavor: MoneyknowFlavor) -> Unit = {},
) {
    commonExtension.apply {
        // 配置维度
        flavorDimensions += MoneyknowFlavorDimension.values().map { it.name }

        // 多渠道配置
        productFlavors {
            MoneyknowFlavor.values().forEach {
                create(it.name) {
...
fun generateFlavor(pkg: String, path: String) {
    val flavor = TypeSpec.enumBuilder(MoneyknowFlavor::class.java.simpleName).apply {
